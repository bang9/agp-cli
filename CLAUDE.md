...
## AGP Workflow Selection

**Choose workflow based on task type:**

### Workflow 1: Modifying Existing Files
- Read `.agp/sessions/{user}/index.md` for current context
- Check ALL `.agp/sessions/*/index.md` for file conflicts before editing
- Read `.agp/project/{file-path}.md` for file-specific knowledge
- Update both source code AND corresponding knowledge file
- Add file to "Active Files" in session with timestamp
- Record progress in "In Progress" section

### Workflow 2: Adding New Features  
- Read `.agp/architecture/overview.md` and `.agp/patterns/overview.md`
- Search `.agp/architecture/feature-domains.md` for similar features
- Create source files following established patterns
- Create `.agp/project/{file-path}.md` for each new file
- Update `.agp/architecture/feature-domains.md` with new features
- Link all new knowledge to existing files in Related sections

### Workflow 3: Session Continuation
- Always start with Step 0 to load current context
- Check "Active Files" are still available for editing
- Continue from "In Progress" tasks in priority order
- Update session immediately after each completed task

## CRITICAL: After Every Task

1. **Update session file**: Mark completed tasks, add new active files
2. **Update knowledge files**: Create/modify `.agp/project/{path}.md` files  
3. **Check for conflicts**: Ensure no other users editing same files
4. **Session compacting**: If 10+ completed tasks, compact to dated file

## Required File Reading

**Always read these files in order:**
1. `.agp/.config.json` (current user/session)
2. `.agp/requirements.md` (project rules - NEVER skip)
3. `.agp/sessions/{user}/index.md` (current session context)
4. `.agp/instructions.md` (full workflow details when needed)

**For file modifications, also read:**
- `.agp/project/{file-path}.md` (file-specific knowledge)
- Related files mentioned in Dependencies section

---
*Auto-generated by `agp connect claude` - Follow these rules strictly*
## Critical AGP Rules

1. **Always follow AGP workflows** from `.agp/instructions.md`
2. **Read `.agp/requirements.md`** before any task - contains user preferences
3. **Update session files** after completing tasks
4. **Update knowledge files** when modifying source code
5. **Follow project requirements** exactly as specified

---
*This section was auto-generated by `agp connect claude`*
